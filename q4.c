...
int diagonalcheck2(int x[][50],int *rowmax,int *colmax);
void displayrowmajor (int a[][50], int rowmax, int colmax);


int main(void)
{
	int a[50][50],rowmax,colmax,r,c,d,d2,biggest;
	readSet (a,&rowmax,&colmax);
	
	r= rowcheck(a,&rowmax,&colmax);
	c= colcheck(a,&rowmax,&colmax);
	d= diagonalcheck(a,&rowmax,&colmax);
	d2= diagonalcheck2(a,&rowmax,&colmax);
	
	biggest=greater(r,c);
	biggest=greater(biggest,d);
	biggest=greater(biggest,d2);
	
	printf("Inputted matrix in row major form:\n");
	displayrowmajor (a, rowmax,colmax);
...
int  diagonalcheck2(int x[][50],int *rowmax,int *colmax)
{
int row=*rowmax-1, col=0, i, j, count=0, tempcount=0, maxcount=0;
